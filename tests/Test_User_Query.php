...
class Test_User_Query extends WP_UnitTestCase {
	...
	public function test_query_without_team_members_is_unchanged() {
		...
		$tid        = WP_User_Teams::create_team( 'Other', 'other', '' );
		WP_User_Teams::set_team_sites( $tid, array( $other_blog => 'editor' ) );
		$uid = self::factory()->user->create( array( 'role' => 'subscriber' ) );
		WP_User_Teams::add_user_to_team( $uid, $tid );
	...
	public function test_query_unaffected_when_blog_id_missing() {
		...
		$tid = WP_User_Teams::create_team( 'NoBlog', 'noblog', 'editor' );
	...
	public function test_team_filter_ignored_outside_users_screen() {
		$tid    = WP_User_Teams::create_team( 'QuietFilter', 'quiet-filter', 'editor' );
		$member = self::factory()->user->create( array( 'role' => 'subscriber' ) );
		$other  = self::factory()->user->create( array( 'role' => 'subscriber' ) );
		WP_User_Teams::add_user_to_team( $member, $tid );
	...
	public function test_views_filter_adds_entry_per_team_with_members() {
		$t_populated = WP_User_Teams::create_team( 'Populated', 'populated', 'editor' );
		$t_empty     = WP_User_Teams::create_team( 'Empty', 'empty', 'editor' );
		WP_User_Teams::add_user_to_team(
	...
	public function test_views_filter_hides_teams_that_dont_cover_current_site() {
		...
		$covers_only_blog3 = WP_User_Teams::create_team( 'Blog3Only', 'blog3-only', '' );
		WP_User_Teams::set_team_sites( $covers_only_blog3, array( $blog3 => 'editor' ) );
		WP_User_Teams::add_user_to_team( self::factory()->user->create(), $covers_only_blog3 );
	...
	public function test_views_filter_base_url_strips_other_view_args() {
		$tid = WP_User_Teams::create_team( 'UrlTeam', 'url-team', 'editor' );
		WP_User_Teams::add_user_to_team( self::factory()->user->create(), $tid );
	...
	public function test_search_finds_team_member_without_native_caps() {
		...
		WP_User_Teams::add_user_to_team( $alice, $tid );
		WP_User_Teams::add_user_to_team( $bob, $tid );
	...
	public function test_role_list_appends_team_role_when_user_lacks_it_natively() {
		$tid = WP_User_Teams::create_team( 'Bonus', 'bonus', 'editor' );
		$uid = self::factory()->user->create( array( 'role' => 'subscriber' ) );
		WP_User_Teams::add_user_to_team( $uid, $tid );
	...
	public function test_role_list_unchanged_when_user_already_has_team_role_natively() {
		$tid = WP_User_Teams::create_team( 'Same', 'same', 'editor' );
