...
class Test_Groups extends WP_UnitTestCase {
	...
	public function test_create_group() {
		...
		$group = Access_Groups::get_group( $id );
	...
	public function test_create_group_auto_slug() {
		$id    = Access_Groups::create_group( 'My Cool Group' );
		$group = Access_Groups::get_group( $id );
	...
	public function test_create_group_unique_slug() {
		$id1 = Access_Groups::create_group( 'Team', 'team' );
		$id2 = Access_Groups::create_group( 'Team', 'team' );

		$g1 = Access_Groups::get_group( $id1 );
		$g2 = Access_Groups::get_group( $id2 );
	...
	public function test_create_group_missing_name() {
		$result = Access_Groups::create_group( '' );
	...
	public function test_create_group_invalid_role_is_cleared() {
		$id    = Access_Groups::create_group( 'Test', 'test', 'not_a_real_role' );
		$group = Access_Groups::get_group( $id );
	...
	public function test_update_group() {
		$id = Access_Groups::create_group( 'Old Name', 'old', 'subscriber' );

		$result = Access_Groups::update_group( $id, array(
			'name' => 'New Name',
			'slug' => 'new-slug',
			'role' => 'editor',
		) );
	...
	public function test_update_group_duplicate_slug_is_auto_incremented() {
		Access_Groups::create_group( 'A', 'taken' );
		$id2 = Access_Groups::create_group( 'B', 'other' );

		$result = Access_Groups::update_group( $id2, array( 'slug' => 'taken' ) );

		$this->assertTrue( $result );
		$group = Access_Groups::get_group( $id2 );
		$this->assertSame( 'taken-2', $group['slug'] );
	}

	public function test_update_group_keeps_own_slug() {
		$id = Access_Groups::create_group( 'Keeper', 'keeper' );

		// Updating without changing the slug should not append a suffix to itself.
		$result = Access_Groups::update_group( $id, array( 'slug' => 'keeper' ) );

		$this->assertTrue( $result );
		$this->assertSame( 'keeper', Access_Groups::get_group( $id )['slug'] );
	}

	public function test_update_nonexistent_group() {
		$result = Access_Groups::update_group( 99999, array( 'name' => 'X' ) );
		$this->assertWPError( $result );
	}

	public function test_delete_group() {
		$id = Access_Groups::create_group( 'Delete Me' );
		$this->assertNotNull( Access_Groups::get_group( $id ) );

		$deleted = Access_Groups::delete_group( $id );
		$this->assertTrue( $deleted );
		$this->assertNull( Access_Groups::get_group( $id ) );
	}

	public function test_delete_nonexistent_group() {
		$this->assertFalse( Access_Groups::delete_group( 99999 ) );
	}

	public function test_get_all_groups() {
		Access_Groups::create_group( 'Alpha' );
		Access_Groups::create_group( 'Beta' );
	...
	public function test_get_group_by_slug() {
		$id = Access_Groups::create_group( 'Find Me', 'find-me' );

		$group = Access_Groups::get_group_by_slug( 'find-me' );
		$this->assertNotNull( $group );
		$this->assertSame( $id, $group['id'] );

		$this->assertNull( Access_Groups::get_group_by_slug( 'nope' ) );
	}
}
